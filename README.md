...
The format is specified in [`skills/mmd/MMD_Format.MD`](skills/mmd/MMD_Format.MD). Agent skills to read, edit, and validate `.mmd` files, or generate maps from Java annotations, live in [`skills/`](skills/) — copy a skill folder into your agent (Cursor: `.cursor/skills/` in a project, or `~/.cursor/skills/` for personal use). After copy, Cursor slash commands such as `/mmd:edit` and `/mmd-annotations:mark` are available; see [`skills/README.md`](skills/README.md). The format spec is part of the `mmd` skill and is included when that folder is copied.
