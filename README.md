...
The format is specified in [`skills/mmd/MMD_Format.MD`](skills/mmd/MMD_Format.MD). Skills that let an AI agent parse and edit `.mmd` files live in [`skills/`](skills/) — copy a skill folder into your agent (Cursor: `.cursor/skills/` in a project, or `~/.cursor/skills/` for personal use). The format spec is part of the `mmd` skill and is included when that folder is copied.
