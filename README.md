...
The format is specified in [`mind-map/MMD_Format.MD`](mind-map/MMD_Format.MD). Skills that let an AI agent parse and edit `.mmd` files live in [`skills/`](skills/) — copy a skill folder into your agent (Cursor: `.cursor/skills/` in a project, or `~/.cursor/skills/` for personal use).
