...
public final class FindFilesForTextPanel extends javax.swing.JPanel {
  ...
  private static final Set<Extra.ExtraType> SEARCH_SCOPE_FOR_MMD_FILE = Collections.unmodifiableSet(
      new HashSet<>(
          Arrays.asList(Extra.ExtraType.NOTE, Extra.ExtraType.LINK, Extra.ExtraType.TOPIC,
  ...
  public FindFilesForTextPanel(@Nonnull final Context context,
                               @Nonnull final NodeFileOrFolder itemToFind,
                               @Nullable final Object okDialogExit) {
    super();
    initComponents();
    ...
    final ComboBoxModel<String> charsets =
    ...
    this.comboCharsets.addActionListener(
        (ActionEvent e) -> LAST_SELECTED_CHARSET = comboCharsets.getSelectedItem().toString());
    ...
    final Runnable runnable = new Runnable() {
      ...
      private void processFile(final NodeFileOrFolder file) {
        ...
        if (!alreadyProcessed
          ...
          MindMap mindMap;
          try (Reader reader = new InputStreamReader(Files.newInputStream(f.toPath()),
          ...
          if (mindMap != null) {
            if (mindMap.findNext(f.getParentFile(), mindMap.getRoot(), searchPatternForMmd, true,
                SEARCH_SCOPE_FOR_MMD_FILE) != null) {
              addFileIntoList(file);
            }
            alreadyProcessed = true;
          }
        ...
        if (!alreadyProcessed) {
          try {
            if (new FileExaminator(f).doesContainData(fileOpBuffer, dataToFindVariant1,
                dataToFindVariant2)) {
              addFileIntoList(file);
            }
          } catch (Exception ex) {
  ...
  private void initComponents() {
    ...
    jLabel1.setText(com.igormaznitsa.sciareto.ui.SrI18n.getInstance().findBundle()
        .getString("panelFindFilesForText.labelTextToSearch")); // NOI18N
    ...
    buttonFind.setText(com.igormaznitsa.sciareto.ui.SrI18n.getInstance().findBundle()
        .getString("panelFindFilesForText.buttonFind")); // NOI18N
    ...
    labelCharset.setText(com.igormaznitsa.sciareto.ui.SrI18n.getInstance().findBundle()
        .getString("panelFindFilesForText.labelCharset")); // NOI18N
    gridBagConstraints = new java.awt.GridBagConstraints();
    ...
    labelLocale.setText(com.igormaznitsa.sciareto.ui.SrI18n.getInstance().findBundle()
        .getString("panelFindFilesForText.labelLocale")); // NOI18N
    gridBagConstraints = new java.awt.GridBagConstraints();
    ...
    checkboxFindInFilenames.setText(com.igormaznitsa.sciareto.ui.SrI18n.getInstance().findBundle()
        .getString("panelFindFilesForText.checkboxFindInFileNames")); // NOI18N
    gridBagConstraints = new java.awt.GridBagConstraints();
    gridBagConstraints.gridx = 4;
    gridBagConstraints.gridy = 0;
  ...
  private void buttonFindActionPerformed(
      java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonFindActionPerformed
    ...
    try {
      ...
      final byte[] str1 = textToSearch.toLowerCase(selectedLocale)
          .getBytes(this.comboCharsets.getSelectedItem().toString());
      final byte[] str2 = textToSearch.toUpperCase(selectedLocale)
          .getBytes(this.comboCharsets.getSelectedItem().toString());
      final Pattern pattern =
          Pattern.compile(Pattern.quote(textToSearch), Pattern.CASE_INSENSITIVE);
      final boolean findInFileNames = this.checkboxFindInFilenames.isSelected();
      LOGGER.info("Start find byte patterns: " + SystemUtils.toString(str1) + ", " +
          SystemUtils.toString(str2));
  ...
  private void listOfFoundElementsMouseClicked(
      java.awt.event.MouseEvent evt) {//GEN-FIRST:event_listOfFoundElementsMouseClicked
    if (evt.getClickCount() > 1 && !evt.isPopupTrigger() &&
        this.listOfFoundElements.getSelectedIndex() >= 0) {
      UiUtils.closeCurrentDialogWithResult(this, this.okDialogExit);
    }
  }//GEN-LAST:event_listOfFoundElementsMouseClicked

  private void listOfFoundElementsMouseMoved(
      java.awt.event.MouseEvent evt) {//GEN-FIRST:event_listOfFoundElementsMouseMoved
