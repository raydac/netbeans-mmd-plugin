...
public final class MainFrame extends javax.swing.JFrame implements Context, PlatformMenuAction,
    ...
    private boolean doClosing() {
        ...
        if (!hasUnsaved && !unsavedFileItems.isEmpty()) {
            try {
                MultiFileContainer.getInstance().write(FILE_NAME_UNSAVED_DOC_CONTAINER, unsavedFileItems).ifPresent(file -> {
                    LOGGER.info("Saved container of unsaved docs " + unsavedFileItems.size() + ": " + file);
                });
            } catch (Exception ex) {
  ...
  private void menuOpenProjectActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuOpenProjectActionPerformed
    ...
    public void editPreferences() {
        ...
        panel.setPreferredSize(new Dimension(Math.min(panelSize.width, (rectangle.width * 3) / 4), Math.min(panelSize.height, (rectangle.height * 3) / 4)));
