...
public class TextEditPanel extends JScrollPane implements TabProvider {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(TextEditPanel.class);
  ...
  public boolean saveDocument() {
    ...
    if (this.title.isChanged()) {
      ...
      if (file == null) {
        file = DialogProviderManager.getInstance().getDialogProvider().msgSaveFileDialog("mmd-panel", "Save Text document", null, true, new TextFileFilter(), "Save");
        if (file == null) {
          return result;
        }
      }
