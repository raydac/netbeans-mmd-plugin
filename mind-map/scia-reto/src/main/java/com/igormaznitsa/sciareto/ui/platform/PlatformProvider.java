...
public final class PlatformProvider {
  ...
  static {
    ...
    if (SystemUtils.IS_OS_MAC_OSX) {
      try {
        detected = (Platform) Class.forName("com.igormaznitsa.sciareto.ui.platform.PlatformMacOSX")
            .getConstructor().newInstance(); //NOI18N
      } catch (Throwable ex) {
      ...
      try {
        detected = (Platform) Class.forName("com.igormaznitsa.sciareto.ui.platform.PlatformLinux")
            .getConstructor().newInstance(); //NOI18N
      } catch (Throwable ex) {
      ...
      try {
        detected = (Platform) Class.forName("com.igormaznitsa.sciareto.ui.platform.PlatformWindows")
            .getConstructor().newInstance(); //NOI18N
      } catch (Throwable ex) {
