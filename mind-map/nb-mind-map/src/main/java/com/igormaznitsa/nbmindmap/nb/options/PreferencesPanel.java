...
public class PreferencesPanel extends AbstractPreferencesPanel implements AdditionalPreferences {
  ...
  protected void beforePanelsCreate(UIComponentFactory uiComponentFactory) {
    ...
    this.checkboxUseRelativePaths.setText(
        bundle.getString("MMDCfgPanel.checkboxRelativePathsForFilesInTheProject.text"));

    this.checkboxUnfoldDropTarget = uiComponentFactory.makeCheckBox();
    this.checkboxUnfoldDropTarget.setText(
        bundle.getString("MMDCfgPanel.checkBoxUnfoldCollapsedTarget.text"));

    this.checkboxCopyParentColors = uiComponentFactory.makeCheckBox();
    this.checkboxCopyParentColors.setText(
        bundle.getString("MMDCfgPanel.checkBoxCopyColorInfoToNewAllowed.text"));

    this.checkboxEnableAutoCreateKnowledgeFolder = uiComponentFactory.makeCheckBox();
    this.checkboxEnableAutoCreateKnowledgeFolder.setText(
        bundle.getString("MMDCfgPanel.checkBoxKnowledgeFolderAutogenerationAllowed.text"));

    this.checkboxWatchFileRefactoring = uiComponentFactory.makeCheckBox();
    this.checkboxWatchFileRefactoring.setText(
        bundle.getString("MMDCfgPanel.checkboxWatchFileRefactoring.text"));

    this.checkboxTurnOffProcessingWhereUsedRefactoring = uiComponentFactory.makeCheckBox();
    this.checkboxTurnOffProcessingWhereUsedRefactoring.setText(
        bundle.getString("MMDCfgPanel.checkboxTurnOffProcessingWhereUsedRefactoring.text"));
    ...
    this.labelIdeShouldBeReload.setText(
        bundle.getString("MMDCfgPanel.labelIdeShouldBeReload.text"));
  }
  ...
  public List<ButtonInfo> findButtonInfo(@Nonnull final UIComponentFactory componentFactory,
    ...
    list.add(ButtonInfo.from(this.loadImage("info16.png"),
        bundle.getString("MMDCfgPanel.buttonAbout.Text"), e -> this.doShowAbout()));
    list.add(ButtonInfo.from(null, null, null, null, DonateButton::new));
    list.add(ButtonInfo.splitter());
    list.add(ButtonInfo.from(this.loadImage("document_export16.png"),
        bundle.getString("MMDCfgPanel.buttonExport.Text"),
        e -> this.exportAsFileDialog(() -> SwingUtilities.getWindowAncestor(this.getPanel()))));
    list.add(ButtonInfo.from(this.loadImage("document_import16.png"),
        bundle.getString("MMDCfgPanel.buttonImport.Text"),
        e -> this.importFromFileDialog(() -> SwingUtilities.getWindowAncestor(this.getPanel()))));
    list.add(ButtonInfo.splitter());
    list.add(ButtonInfo.from(this.loadImage("stop16.png"),
        bundle.getString("MMDCfgPanel.buttonReset.Text"),
  ...
  public void onSave(@Nonnull final MindMapPanelConfig config) {
    ...
    config.setOptionalProperty(PROPERTY_USE_INTERNAL_BROWSER,
        this.checkboxPreferInternalBrowser.isSelected());
    config.setOptionalProperty(PROPERTY_MAKE_RELATIVE_PATHS_TO_PROJECT_ROOT,
        this.checkboxUseRelativePaths.isSelected());
    config.setOptionalProperty(PROPERTY_UNFOLD_COLLAPSED_DROP_TARGET,
        this.checkboxUnfoldDropTarget.isSelected());
    config.setOptionalProperty(PROPERTY_COPY_PARENT_COLORS_TO_NEW_CHILD,
        this.checkboxCopyParentColors.isSelected());
    config.setOptionalProperty(PROPERTY_KNOWLEDGE_FOLDER_ALLOWED,
        this.checkboxEnableAutoCreateKnowledgeFolder.isSelected());
    config.setOptionalProperty(PROPERTY_WATCH_FILE_REFACTORING,
        this.checkboxWatchFileRefactoring.isSelected());
    config.setOptionalProperty(PROPERTY_TURN_OFF_PROCESSING_WHERE_USED,
        this.checkboxTurnOffProcessingWhereUsedRefactoring.isSelected());
  }
  ...
  public void onLoad(@Nonnull final MindMapPanelConfig config) {
    ...
    this.checkboxPreferInternalBrowser.setSelected(
        config.getOptionalProperty(PROPERTY_USE_INTERNAL_BROWSER, false));
    this.checkboxUseRelativePaths.setSelected(
        config.getOptionalProperty(PROPERTY_MAKE_RELATIVE_PATHS_TO_PROJECT_ROOT, true));
    this.checkboxUnfoldDropTarget.setSelected(
        config.getOptionalProperty(PROPERTY_UNFOLD_COLLAPSED_DROP_TARGET, true));
    this.checkboxCopyParentColors.setSelected(
        config.getOptionalProperty(PROPERTY_COPY_PARENT_COLORS_TO_NEW_CHILD, true));
    this.checkboxEnableAutoCreateKnowledgeFolder.setSelected(
        config.getOptionalProperty(PROPERTY_KNOWLEDGE_FOLDER_ALLOWED, false));
    this.checkboxWatchFileRefactoring.setSelected(
        config.getOptionalProperty(PROPERTY_WATCH_FILE_REFACTORING, false));
    this.checkboxTurnOffProcessingWhereUsedRefactoring.setSelected(
