...
public class MindMapTest {
  ...
  public void testMindMapParse_Error_Empty() throws Exception {
    new MindMap(new StringReader(""));
  }

  @Test(expected = IllegalArgumentException.class)
  public void testMindMapParse_Error_JustTextString() throws Exception {
    new MindMap(new StringReader("Who is here"));
  }
  ...
  public void testFindNext_Null() throws Exception {
    ...
    assertEquals("Mercury",
        map.findNext(null, null, Pattern.compile(Pattern.quote("cury")), true, null).getText());
  }
  ...
  public void testFindPrev_Null() throws Exception {
    final MindMap map = new MindMap(new StringReader(
        "test\n---\n# Solar\n## Mercury\n## Venus\n## Earth\n### Moon\n## Mars\n### Phobos\n### Deimos"));
    assertEquals("Solar",
        map.findPrev(null, null, Pattern.compile(Pattern.quote("lar")), true, null).getText());
  }
  ...
  public void testFindPrev_NonNull() throws Exception {
    final MindMap map = new MindMap(new StringReader(
        "test\n---\n# Solar\n## Mercury\n## Venus\n## Earth\n### Moon\n## Mars\n### Phobos\n### Deimos"));
    final Topic base =
        map.findNext(null, null, Pattern.compile(Pattern.quote("Deimos")), true, null);
  ...
  public void testMindMapParse_NoAttributes() throws Exception {
    ...
    assertNull(map.findAttribute("test"));
  ...
  public void testMindMapParse_oneAttributeDoubleNewLine() throws Exception {
    final MindMap map = new MindMap(new StringReader("lkf\n> test=`Hi`\n\n---\n# Hello"));
    assertEquals("Hi", map.findAttribute("test"));
    assertEquals("Hello", map.getRoot().getText());
  }
  ...
  public void testMindMapParse_overridenAttributes_SingleNewLine() throws Exception {
    final MindMap map =
        new MindMap(new StringReader("lkf\n> test=`Hi`\n> test=`Lo`\n---\n# Hello"));
    assertEquals("Lo", map.findAttribute("test"));
    assertEquals("Hello", map.getRoot().getText());
  }
  ...
  public void testMindMapWrite_WithoutAttributes() throws Exception {
    ...
    map.write(writer);
    assertEquals("[Scia Reto](https://sciareto.org) mind map   \n> __version__=`1.1`\n---\n",
        writer.toString());
  }
  ...
  public void testIteration_TwoLevel() throws Exception {
    ...
    assertArrayEquals(
        new String[] {"root", "child1", "child1.1", "child1.2", "child2", "child2.1", "child2.2"},
