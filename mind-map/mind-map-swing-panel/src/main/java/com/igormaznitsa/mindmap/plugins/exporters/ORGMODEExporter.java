...
public class ORGMODEExporter extends AbstractExporter {
  ...
  public void doExport(@Nonnull final MindMapPanel panel, @Nonnull final JComponent options, @Nullable final OutputStream out) throws IOException {
    ...
    state.append("#+CREATOR: ").append("Generated by <a href=\\\"https://github.com/raydac/netbeans-mmd-plugin\\\">NB Mind Map Plugin</a>").nextLine();//NOI18N
    ...
    if (theOut == null) {
      fileToSaveMap = MindMapUtils.selectFileToSaveForFileFilter(panel, Texts.getString("ORGMODEExporter.saveDialogTitle"), ".org", Texts.getString("ORGMODEExporter.filterDescription"), Texts.getString("ORGMODEExporter.approveButtonText"));
      fileToSaveMap = MindMapUtils.checkFileAndExtension(panel, fileToSaveMap, ".org");//NOI18N
  ...
  private static String formatTimestamp(final long time) {
    ...
    result.append(calendar.get(Calendar.YEAR)).append('-').append(ensureNumberFormatting(2, calendar.get(Calendar.MONTH) + 1)).append(ensureNumberFormatting(2, calendar.get(Calendar.DAY_OF_MONTH)));
    result.append(' ');
