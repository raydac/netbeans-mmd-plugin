...
import com.igormaznitsa.nbmindmap.model.ExtraLinkable;
...
public class FreeMindExporter extends AbstractMindMapExporter {
  ...
  private static void writeTopicRecursively(final Topic topic, final Configuration cfg, int shift, final State state) {
    ...
    if (htmlTextForNode.length() > 0) {
      state.append(childShift).append("<richcontent TYPE=\"NOTE\">").append("<html><head></head><body>"+htmlTextForNode.toString()+"</body></html>").append("</richcontent>").nextLine();//NOI18N
    }
  ...
  public void doExport(final MindMapPanel currentPanel) throws IOException {
    ...
    state.append("<map version=\"1.0.1\" background_color=\"").append(color2html(currentPanel.getConfiguration().getPaperColor())).append("\">").nextLine();//NOI18N
