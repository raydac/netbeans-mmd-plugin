...
public class TextExporter extends AbstractMindMapExporter {
  ...
  private static void writeTopic(final Topic topic, final char ch, final int shift, final State state) {
    ...
    if (file != null) {
      ...
      state.append(shiftString("FILE: ", ' ', shift)).append(fileURI.isAbsolute() ? Utilities.toFile(file.getValue()).getAbsolutePath() : fileURI.toString()).nextLine();//NOI18N
      extrasPrinted = true;
    }
  ...
  public void doExport(final MindMapPanel currentPanel) throws IOException {
    ...
    File fileToSaveText = new FileChooserBuilder("user-dir")//NOI18N
      ...
      public boolean accept(File f) {
        return f.isDirectory() || (f.isFile() && f.getName().toLowerCase(Locale.ENGLISH).endsWith(".txt"));//NOI18N
      }
    ...
    fileToSaveText = checkFile(fileToSaveText, ".txt");//NOI18N
    
    if (fileToSaveText != null) {
      FileUtils.writeStringToFile(fileToSaveText, text, "UTF-8");//NOI18N
    }
  }
